...
void main() {
  ...
  group('migrate CLI', () {
    test('ARB converts ICU plurals to objects and flags select', () async {
      final out = Directory(
        p.join(root, 'test', 'migrate', 'out_arb_${DateTime.now().microsecondsSinceEpoch}'),
      )..createSync();
      addTearDown(() {
    ...
    test('easy_localization converts positional placeholders', () async {
      final out = Directory(
        p.join(root, 'test', 'migrate', 'out_easy_${DateTime.now().microsecondsSinceEpoch}'),
      )..createSync();
      addTearDown(() {
        if (out.existsSync()) out.deleteSync(recursive: true);
      });
    ...
    test('slang converts \$name to {name}', () async {
      final out = Directory(
        p.join(root, 'test', 'migrate', 'out_slang_${DateTime.now().microsecondsSinceEpoch}'),
      )..createSync();
      addTearDown(() {
        if (out.existsSync()) out.deleteSync(recursive: true);
      });
    ...
    test('dry-run does not write files', () async {
      final out = Directory(
        p.join(root, 'test', 'migrate', 'out_dry_${DateTime.now().microsecondsSinceEpoch}'),
      );
    ...
    test('refuses non-empty output without --force', () async {
      final out = Directory(
        p.join(root, 'test', 'migrate', 'out_force_${DateTime.now().microsecondsSinceEpoch}'),
      )..createSync();
      addTearDown(() {
        if (out.existsSync()) out.deleteSync(recursive: true);
      });
      ...
      expect(result.stdout.toString() + result.stderr.toString(), contains('--force'));
    });
  });
}
